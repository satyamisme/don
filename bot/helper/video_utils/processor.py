...
async def select_streams(media_info):
    ...
    if selected_audio_stream:
        audio_streams.append(selected_audio_stream)

    # Subtitle stream selection (allow-list)
    for stream in media_info.get('streams', []):
